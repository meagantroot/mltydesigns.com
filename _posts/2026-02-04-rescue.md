...
In order to read the database files from the failed drive we need a database engine up and running locally. This guide does not cover installing MariaDB, but will show you how to setip a temporary configuration that reads from the backup copy we took off the failed drive. 
